...
#ifndef SQL_SERVER_DBINDER_CLASS_H
#define SQL_SERVER_DBINDER_CLASS_H

class SqlServerRootClass;

class SqlServerDBinderClass {
public:
    SqlServerDBinderClass(SqlServerRootClass *root_object_val);
    ~SqlServerDBinderClass();

private:
    SqlServerRootClass *rootObject_;

    const char *objectName() { return "SqlServerDBinderClass"; }
